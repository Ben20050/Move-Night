...
def string_checker(question, num_letters, valid_responses):
    ...
    while True:
        ...
        for r_item in valid_responses:
            if response == r_item[:num_letters] or response == r_item:
                return r_item
...
while tickets_sold < MAX_TICKETS:
    ...
    if name == 'xxx' and len(all_names) > 0:
        ...
        print("You must sell at least ONE ticket before leaving")
        continue

    age = num_check("Age: ")

    # checks user is between 12 and 120 (inclusive)
    if 12 <= age <= 120:
        pass
    elif age < 12:
    ...
    if pay_method == "cash":
        ...
        # Calculates 5% surcharge if users are paying by credit card
...
# Creates data frame from dictionary to organise information
...
# Displays the profit per ticket
